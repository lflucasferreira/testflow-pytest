# Testes de API — Autenticação (Login)

**Arquivo-fonte:** [`../../../../tests/api/test_auth_api.py`](../../../../tests/api/test_auth_api.py)

---

## Propósito

Este módulo valida o endpoint **`POST /api/auth/login`** e fluxos relacionados na interface de login. Combina:

1. **Testes de API pura** — status, headers, corpo JSON, latência, token
2. **Testes de credenciais inválidas e payload malformado**
3. **Testes híbridos UI + rede** — interceptação de rotas (`page.route`) e stub de respostas

É referência para estudantes de **APIRequestContext**, fixtures com escopo de classe e **network interception** no Playwright.

---

## Pré-requisitos

| Item | Descrição |
|------|-----------|
| API | Servidor com rota `/api/auth/login` |
| Config | `BASE_URL`, `DEMO_EMAIL`, `DEMO_PASSWORD` em `support/config.py` |
| Fixtures globais | `api_request`, `playwright`, `page` |
| Execução API-only | Não exige browser para a maioria dos testes |

```bash
pytest tests/api/test_auth_api.py -v
pytest tests/api/test_auth_api.py -m "api and smoke"
pytest tests/api/test_auth_api.py -m critical
```

---

## Markers utilizados

| Marker | Uso |
|--------|-----|
| `api` | Todos os testes REST / intercept |
| `regression` | Suite completa |
| `smoke` | Caminhos felizes críticos (200, token) |
| `critical` | `test_body_has_token_as_non_empty_string` |

---

## Visão geral da estrutura

```
test_auth_api.py
├── imports
├── constantes: ENDPOINT, VALID
├── fixture: valid_login_response (scope=class)
├── TestAuthLoginValidCredentials
├── TestAuthLoginInvalidCredentials
├── TestAuthLoginMalformedRequest
└── TestAuthLoginInterceptFlow
```

---

## Imports — bloco a bloco

### `import re`

Regex para asserção de URL na página de login após erro simulado (`/web/login\.html`).

---

### `import time`

Medição de latência com **`time.monotonic()`** — relógio monotônico, imune a ajustes de relógio do sistema (preferível a `time.time()` para benchmarks).

---

### `import pytest`

Fixtures, markers, organização em classes.

---

### `from playwright.sync_api import APIRequestContext, Page, expect`

| Símbolo | Uso neste arquivo |
|---------|-------------------|
| `APIRequestContext` | Cliente HTTP do Playwright — POST/GET sem browser |
| `Page` | Testes de intercept na UI de login |
| `expect` | Asserção de URL e visibilidade na UI |

**Conceito Playwright:** `APIRequestContext` compartilha cookies/storage opcionalmente com browser, mas aqui usa contextos isolados para testes determinísticos.

---

### `from support.config import BASE_URL, DEMO_EMAIL, DEMO_PASSWORD`

Centraliza ambiente e credenciais demo válidas.

---

## Constantes de módulo

### `ENDPOINT = "/api/auth/login"`

Path relativo concatenado com `BASE_URL` nas requisições.

---

### `VALID = {"email": DEMO_EMAIL, "password": DEMO_PASSWORD}`

Payload de login válido reutilizado em todo o arquivo — **single source of truth** para happy path.

---

## Fixture: `valid_login_response`

```python
@pytest.fixture(scope="class")
def valid_login_response(playwright) -> dict:
    ctx = playwright.request.new_context(base_url=BASE_URL)
    try:
        start = time.monotonic()
        response = ctx.post(f"{BASE_URL}{ENDPOINT}", data=VALID)
        elapsed_ms = (time.monotonic() - start) * 1000
        return {
            "status": response.status,
            "headers": response.headers,
            "body": response.json(),
            "duration_ms": elapsed_ms,
        }
    finally:
        ctx.dispose()
```

### Conceitos PyTest

| Aspecto | Detalhe |
|---------|---------|
| `scope="class"` | Uma única chamada HTTP por **classe** de teste que usa a fixture — economiza tempo |
| `playwright` | Fixture de sessão do pytest-playwright |
| `try/finally` + `dispose()` | Garante liberação do contexto HTTP |

### Retorno

Dicionário com status, headers, body parseado e duração em ms — testes derivados só **assertam** sem repetir POST.

**Given (para TestAuthLoginValidCredentials):** resposta de login válido já materializada.

---

## Classe `TestAuthLoginValidCredentials`

Validações do caminho feliz com credenciais demo.

---

### `test_returns_status_200`

```python
@pytest.mark.smoke
def test_returns_status_200(self, valid_login_response: dict) -> None:
    assert valid_login_response["status"] == 200
```

**Then:** HTTP 200 OK.

---

### `test_content_type_is_application_json`

```python
def test_content_type_is_application_json(self, valid_login_response: dict) -> None:
    assert "application/json" in valid_login_response["headers"].get("content-type", "")
```

**Then:** header `content-type` contém JSON. Uso de `.get(..., "")` evita `KeyError`.

---

### `test_responds_within_2000ms`

```python
def test_responds_within_2000ms(self, api_request: APIRequestContext) -> None:
    start = time.monotonic()
    response = api_request.post(f"{BASE_URL}{ENDPOINT}", data=VALID)
    elapsed_ms = (time.monotonic() - start) * 1000
    assert response.status == 200
    assert elapsed_ms < 2000
```

**When:** POST fresco via fixture `api_request` (function scope — não reutiliza cache da class fixture).

**Then:** 200 e resposta em menos de 2 segundos — teste de **SLA** básico.

---

### `test_body_has_token_as_non_empty_string`

```python
@pytest.mark.smoke
@pytest.mark.critical
def test_body_has_token_as_non_empty_string(self, valid_login_response: dict) -> None:
    token = valid_login_response["body"]["token"]
    assert isinstance(token, str)
    assert token
```

**Then:** campo `token` existe, é string não vazia. Marker `critical` para pipelines prioritários.

---

### `test_body_has_user_object_or_token_only_response`

```python
def test_body_has_user_object_or_token_only_response(self, valid_login_response: dict) -> None:
    body = valid_login_response["body"]
    if body.get("user"):
        assert isinstance(body["user"].get("email"), str)
        assert body["user"]["email"]
    else:
        assert isinstance(body["token"], str)
        assert body["token"]
```

**Padrão tolerante:** API pode retornar `{ token, user }` ou só `{ token }` — teste aceita ambos contratos.

---

### `test_user_email_matches_the_login_email_when_user_is_present`

```python
def test_user_email_matches_the_login_email_when_user_is_present(
    self, valid_login_response: dict
) -> None:
    body = valid_login_response["body"]
    if body.get("user") and body["user"].get("email"):
        assert body["user"]["email"] == VALID["email"]
```

**Then condicional:** só asserta consistência se objeto `user.email` existir.

---

### `test_token_can_authenticate_a_subsequent_request`

```python
def test_token_can_authenticate_a_subsequent_request(
    self, api_request: APIRequestContext, valid_login_response: dict
) -> None:
    token = valid_login_response["body"]["token"]
    response = api_request.get(
        f"{BASE_URL}/api/users",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert response.status == 200
```

| Fase | Descrição |
|------|-----------|
| **Given** | Token obtido no login |
| **When** | GET `/api/users` com header Bearer |
| **Then** | 200 — token é funcional, não apenas presente |

**Conceito OAuth/JWT:** padrão `Authorization: Bearer <token>`.

---

## Classe `TestAuthLoginInvalidCredentials`

---

### `test_returns_401_for_wrong_password`

```python
def test_returns_401_for_wrong_password(self, api_request: APIRequestContext) -> None:
    response = api_request.post(
        f"{BASE_URL}{ENDPOINT}",
        data={"email": VALID["email"], "password": "wrongpassword"},
    )
    assert response.status == 401
```

Email válido + senha errada → **401 Unauthorized**.

---

### `test_returns_401_for_unknown_email`

```python
def test_returns_401_for_unknown_email(self, api_request: APIRequestContext) -> None:
    response = api_request.post(
        f"{BASE_URL}{ENDPOINT}",
        data={"email": "nobody@example.com", "password": VALID["password"]},
    )
    assert response.status == 401
```

Usuário inexistente → 401 (não revela se email existe — boa prática de segurança).

---

### `test_error_response_has_a_non_empty_error_or_message_field`

```python
def test_error_response_has_a_non_empty_error_or_message_field(
    self, api_request: APIRequestContext
) -> None:
    response = api_request.post(
        f"{BASE_URL}{ENDPOINT}",
        data={"email": VALID["email"], "password": "wrong"},
    )
    body = response.json()
    err_text = body.get("message") or (body.get("error") or {}).get("message")
    assert isinstance(err_text, str)
    assert err_text
```

**Contrato de erro flexível:** aceita `message` top-level ou `error.message` aninhado.

---

## Classe `TestAuthLoginMalformedRequest`

Validação de entrada — espera **4xx** (400–422).

---

### `test_returns_4xx_when_body_is_empty`

```python
def test_returns_4xx_when_body_is_empty(self, api_request: APIRequestContext) -> None:
    response = api_request.post(f"{BASE_URL}{ENDPOINT}", data={})
    assert 400 <= response.status <= 422
```

---

### `test_returns_4xx_when_email_is_missing`

```python
def test_returns_4xx_when_email_is_missing(self, api_request: APIRequestContext) -> None:
    response = api_request.post(f"{BASE_URL}{ENDPOINT}", data={"password": VALID["password"]})
    assert 400 <= response.status <= 422
```

---

### `test_returns_4xx_when_password_is_missing`

```python
def test_returns_4xx_when_password_is_missing(self, api_request: APIRequestContext) -> None:
    response = api_request.post(f"{BASE_URL}{ENDPOINT}", data={"email": VALID["email"]})
    assert 400 <= response.status <= 422
```

**Faixa 400–422:** cobre Bad Request, Unprocessable Entity conforme implementação do sandbox.

---

## Classe `TestAuthLoginInterceptFlow`

Testes **browser + rede** — não usam só `api_request`.

---

### `test_api_toggle_sends_post_with_correct_payload_and_token_response`

```python
def test_api_toggle_sends_post_with_correct_payload_and_token_response(
    self, page: Page
) -> None:
    captured: dict = {}

    def handle_route(route) -> None:
        post_data = route.request.post_data_json or {}
        captured["email"] = post_data.get("email")
        route.continue_()

    page.route(f"**{ENDPOINT}", handle_route)
    page.goto("/web/login.html")
    page.get_by_test_id("login-use-api").evaluate("el => el.click()")
    page.get_by_test_id("login-email").fill(VALID["email"])
    page.get_by_test_id("login-password").fill(VALID["password"])

    with page.expect_response(f"**{ENDPOINT}") as response_info:
        page.get_by_test_id("login-submit").click()

    response = response_info.value
    assert captured.get("email") == VALID["email"]
    assert response.status == 200
    body = response.json()
    assert isinstance(body.get("token"), str)
    assert body["token"]
```

| Fase | Descrição |
|------|-----------|
| **Given** | Handler registra email do POST sem bloquear (`continue_`) |
| **When** | UI login com modo API ativado |
| **Then** | Payload correto; resposta real 200 com token |

**Conceitos Playwright:**

- **`page.route`**: intercepta requests que batem no pattern `**/api/auth/login`
- **`post_data_json`**: body JSON parseado da requisição
- **`page.expect_response`**: context manager que captura response após ação
- **`evaluate("el => el.click()")`**: clique via JS quando clique normal falha (checkbox/toggle oculto)

---

### `test_stubbed_500_keeps_user_on_login_page_without_crashing`

```python
def test_stubbed_500_keeps_user_on_login_page_without_crashing(self, page: Page) -> None:
    page.route(
        f"**{ENDPOINT}",
        lambda route: route.fulfill(
            status=500,
            content_type="application/json",
            body='{"error":"Internal Server Error"}',
        ),
    )
    page.goto("/web/login.html")
    page.get_by_test_id("login-use-api").evaluate("el => el.click()")
    page.get_by_test_id("login-email").fill(VALID["email"])
    page.get_by_test_id("login-password").fill(VALID["password"])

    with page.expect_response(f"**{ENDPOINT}"):
        page.get_by_test_id("login-submit").click()

    expect(page).to_have_url(re.compile(r"/web/login\.html"))
    result = page.get_by_test_id("login-result")
    if result.is_visible():
        expect(result).not_to_be_empty()
```

| Fase | Descrição |
|------|-----------|
| **Given** | API de login sempre retorna 500 simulado |
| **When** | Submit do formulário |
| **Then** | Permanece em login; se mensagem de resultado aparecer, não está vazia |

**Conceito:** `route.fulfill` **substitui** resposta — teste de resiliência da UI a falha de backend.

---

## Tabela resumo — tipos de teste

| Classe | Camada | Fixture principal |
|--------|--------|-------------------|
| ValidCredentials | API | `valid_login_response` |
| InvalidCredentials | API | `api_request` |
| MalformedRequest | API | `api_request` |
| InterceptFlow | UI + Network | `page` |

---

## Checklist de aprendizado

- [ ] Explicar `scope="class"` vs function scope em fixtures HTTP
- [ ] Diferenciar `route.continue_()` vs `route.fulfill()`
- [ ] Descrever header Bearer em request autenticado
- [ ] Justificar faixa `400 <= status <= 422`
- [ ] Executar `-m "api and critical"` e interpretar resultado
